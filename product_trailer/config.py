...
class Config():
    ...
    def import_config(self) -> bool:
        ...
        if not self.database_path.is_dir():
            shutil.copytree('./profiles/default_profile/database', self.profile_path / 'database')
        if not self.config_path.is_dir():
            shutil.copytree('./profiles/default_profile/config', self.profile_path / 'config')
        ...
        # Custom tools
        processing = importlib.import_module(f'profiles.{self.profile_name}.config.processing')
    ...
    def postprocess(self, tracked_items: pd.DataFrame) -> bool:
        custom_postprocessing = importlib.import_module(f'profiles.{self.profile_name}.config.postprocessing')
    ...
    def fetch_saved_items(self) -> None | pd.DataFrame:
        possible_db = list(self.database_path.glob(self.db_config['filename_tracked_items'] + '*'))
    ...
    def save_items(self, tracked_items: pd.DataFrame, date_range_db: str) -> None:
        ...
        new_db_filename = self.database_path / f"{self.db_config['filename_tracked_items']} {date_range_db} (saved {datetime_db_creation}).pkl"
    ...
    def save_movements(self, list_computed_MVTS: pd.DataFrame, date_range_db: str) -> None:
        if self.db_config['save_movements']:
            ...
            if len(possible_mvt_db) == 0:
                ...
                new_MVT_DB = pd.concat([pd.read_pickle(prev_dbfilename_MVTS), MVT_DB], axis=0)
            
            datetime_db_creation = datetime.today().strftime("%Y-%m-%d %Hh%M")
            new_db_mvt_filepath = self.database_path / f"{self.db_config['filename_movements']} {date_range_db} (saved {datetime_db_creation}).pkl"
